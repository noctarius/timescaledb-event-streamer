...
func PlanTupleDecoder(relation *RelationMessage) (TupleDecoderPlan, error) {
	...
	for _, column := range relation.Columns {
		...
		if pgxType, ok := typeMap.TypeForOID(column.DataType); ok {
			codec = func(data []byte, binary bool) (any, error) {
				dataformat := int16(pgtype.TextFormatCode)
				if binary {
					dataformat = pgtype.BinaryFormatCode
				}
				return pgxType.Codec.DecodeValue(typeMap, column.DataType, dataformat, data)
			}
		}
...
func (tdp *tupleDecoderPlan) Decode(tupleData *pglogrepl.TupleData) (map[string]any, error) {
	if tupleData == nil {
