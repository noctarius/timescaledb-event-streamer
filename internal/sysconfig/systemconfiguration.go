...
func (sc *SystemConfig) defaultEventEmitter(schemaRegistry *schema.Registry,
	...
	filters, err := eventfiltering.NewEventFilter(sc.Config.Sink.Filters)
	if err != nil {
		return nil, err
	}

	return eventemitting.NewEventEmitter(schemaRegistry, topicNameGenerator, transactionMonitor, s, filters), nil
}
